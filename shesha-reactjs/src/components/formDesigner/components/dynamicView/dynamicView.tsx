...
export const DynamicView: FC<DynamicViewProps> = model => {
  ...
  const propsToRender = useMemo<IPropertyMetadata[]>(() => {
    ...
    const propertiesToMap = currentMeta.properties.filter(
      property =>
        property.isVisible &&
        !property.isFrameworkRelated &&
        !staticComponentBindings.includes(camelcaseDotNotation(property.path))
    );
    return propertiesToMap;
  }, [staticComponents, currentMeta]);

  const dynamicComponents = useMemo(() => {
    const components = propsToRender
      .map(prop => {
        const component = createComponentModelForDataProperty(toolboxComponentGroups, prop);
        if (component) component.isDynamic = true;
        return component;
      })
      .filter(c => Boolean(c));
    return components;
  ...
  return <DynamicContainer components={dynamicComponents} />;
};
